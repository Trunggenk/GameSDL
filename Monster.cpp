...
void Monster::MonsterMove(Map& mymap)
{
	x_value = 0;
...
void Monster::CheckVaCham(Map& mymap)
{
...
{
	if (x_pos <= mymap.start_x_ + SCREEN_WIDTH*2/3 || Meet_Sasuke) { Meet_Sasuke = true; return true;}
	else return false;
}
...
void Monster::Action(Player sake)
{
	if (y_pos == sake.y_pos)
	{
		if (sake.x_pos < x_pos && type_input.normal_attack == 0) { Direction = Left; type_input.run = 1; }
		else if (sake.x_pos >= x_pos && type_input.normal_attack == 0) { Direction = Right; type_input.run = 1; }
		if (sake.x_pos - TILE_SIZE <= x_pos && y_pos == sake.y_pos && Direction == Right)
		{
			type_input.normal_attack = 1; type_input.run = 1;
		}
