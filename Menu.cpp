...
void Menu::HandleInputMenu(SDL_Event events, SDL_Renderer *renderer) {
    switch (events.type) {
        case SDL_MOUSEMOTION: {
            if (type_menu == StartMenu) {
                if (events.motion.x <= 3 * TILE_SIZE + start_game.GetWidth() && events.motion.x >= 3 * TILE_SIZE &&
                    events.motion.y >= 5 * TILE_SIZE && events.motion.y <= 5 * TILE_SIZE + start_game.GetHeight()) {
                ...
                if (events.motion.x <= 3 * TILE_SIZE + exit_game.GetWidth() && events.motion.x >= 3 * TILE_SIZE &&
                    events.motion.y >= 6.5 * TILE_SIZE && events.motion.y <= 6.5 * TILE_SIZE + exit_game.GetHeight()) {
                    in_exit_button = true;
                    start_game.SetColor(Text::BLACK_TEXT);
            ...
            if (type_menu == EndMenu) {
                if (events.motion.x <= 10 * TILE_SIZE + play_again.GetWidth() && events.motion.x >= 10 * TILE_SIZE &&
                    events.motion.y >= 5 * TILE_SIZE && events.motion.y <= 5 * TILE_SIZE + play_again.GetHeight()) {
                ...
                if (events.motion.x <= 10 * TILE_SIZE + exit_game.GetWidth() && events.motion.x >= 10 * TILE_SIZE &&
                    events.motion.y >= 6.5 * TILE_SIZE && events.motion.y <= 6.5 * TILE_SIZE + exit_game.GetHeight()) {
                    in_exit_button = true;
                    play_again.SetColor(Text::BLACK_TEXT);
                    exit_game.SetColor(Text::RED_TEXT);
